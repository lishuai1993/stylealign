...
def get_data_loader_face(conf):
    ...
    img_list = conf['img_list']
    input_dim_b = conf['input_dim_b']
    zoom_min = conf['zoom_min']
    zoom_max = conf['zoom_max']
    FaceBoundary = FaceDataset(img_shape = [width,height,3], train = train, sigma = sigma,root_dir= root_dir, img_list= img_list, input_dim_b=input_dim_b, zoom_min=zoom_min, zoom_max=zoom_max)
    loader = DataLoader(dataset = FaceBoundary, batch_size=batch_size, shuffle = train, drop_last=True, num_workers=num_workers)
    return loader

def get_test_loader_face(conf):
    batch_size = conf['display_size']
    num_workers = conf['num_workers']
    train = False
    if 'new_size' in conf:
        new_size_a = new_size_b = conf['new_size']
